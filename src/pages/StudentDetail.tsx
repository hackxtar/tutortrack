...
import { ArrowLeft, User, Phone, Mail, FileText, CheckCircle, Clock, MessageCircle, Check, Sparkles, Loader2, AlertCircle } from 'lucide-react';
import { Student, ClassNote, FollowUp } from '../types';
import { studentsApi, classNotesApi, followUpsApi } from '../api/services';
...
export function StudentDetail({ 
  ...
  const fetchStudentData = async () => {
    ...
    setError(null);
    try {
      const [sRes, nRes, fRes] = await Promise.all([
        studentsApi.getById(studentId),
        classNotesApi.list({ studentId }),
        followUpsApi.list({ studentId }),
      ]);

      if (sRes?.data?.student) setStudent(sRes.data.student);
      if (nRes?.data) setStudentNotes(nRes.data);
      if (fRes?.data) setStudentFollowUps(fRes.data);
    } catch (err: any) {
  ...
  if (loading && !student) {
    return (
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-16 flex flex-col items-center justify-center gap-3 text-slate-500 font-['Inter']">
  ...
  if (error || !student) {
    return (
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12 flex flex-col items-center gap-4 font-['Inter']">
        ...
        <button onClick={onBack} className="px-4 py-2 bg-slate-900 text-white text-xs font-semibold rounded-lg hover:bg-slate-800 cursor-pointer">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-8 flex flex-col gap-6 relative font-['Inter']">
      {/* Toast Feedback */}
      {toastMessage && (
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex items-start sm:items-center gap-5">
            <div className="relative flex-shrink-0">
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center font-['Plus_Jakarta_Sans'] text-2xl font-bold shadow-xs ${student.avatarColor || 'bg-indigo-100 text-indigo-700'}`}>
            ...
            <div className="flex flex-col min-w-0 gap-1.5">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500 flex-wrap font-medium">
                <span className="flex items-center gap-1">
                  ...
                  Teacher: <strong className="text-slate-900">{student.teacher || 'Amit Sharma'}</strong>
                </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-slate-100">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-1.5">
              <span className="font-semibold text-sm text-slate-900 truncate">{student.parentName || 'Parent'}</span>
            </div>
            <div className="flex items-center gap-2 mt-1">
              <button 
                onClick={() => handleWhatsApp(student.parentPhone, `Hi ${student.parentName || 'Parent'}, Amit here from TutorTrack with an update on ${student.name}'s progress.`)}
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-slate-500 font-medium">{student.schedule || 'Regular'}</span>
          </div>

          <div className="flex flex-col gap-1">
             <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Subscription &amp; Milestone</span>
             <div className="flex items-center gap-2">
                <span className="font-semibold text-sm text-slate-900">Recorded Sessions: {studentNotes.length}</span>
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="flex items-center justify-between">
             <div className="flex items-center gap-3">
               ...
               <span className="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-[10px] font-bold">{studentNotes.length} sessions recorded</span>
             </div>
             <button
               onClick={() => setIsNoteModalOpen(true)}
          ...
          {studentNotes.length === 0 ? (
            <div className="bg-white rounded-xl p-8 border border-slate-100 text-center text-sm text-slate-400">
              No class notes recorded yet for this student.
            </div>
          ) : (
              ...
              <div key={note.id} className="bg-white rounded-xl p-5 border border-slate-100 shadow-xs flex flex-col gap-4 relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold self-start sm:self-center ${
                    note.understanding === 'Excellent' ? 'bg-emerald-100 text-emerald-800' :
                    note.understanding === 'Good Understanding' ? 'bg-blue-100 text-blue-800' :
                    'bg-amber-100 text-amber-800'
                  }`}>
                    {note.understanding || 'Good'} Progress
                  </span>
                </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-3">
             <div className="flex items-center justify-between">
               ...
               <span className="px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 text-[10px] font-bold">{studentFollowUps.length}</span>
             </div>
             
             {studentFollowUps.length === 0 ? (
               <div className="bg-white rounded-xl p-4 border border-slate-100 text-xs text-slate-400 italic">
                 No active follow-ups for this student.
               </div>
             ) : (
               studentFollowUps.map(fu => (
                 <div key={fu.id} className="bg-white rounded-xl p-4 shadow-xs border border-slate-100 flex flex-col gap-2 relative overflow-hidden">
                   <div className="flex items-center justify-between">
                     ...
                     <span className="text-[11px] font-semibold text-rose-600">Scheduled: {fu.due}</span>
                   </div>
                   <h4 className="font-semibold text-sm text-slate-900">{fu.objective || 'Follow-up Task'}</h4>
                   <p className="text-xs text-slate-500 leading-relaxed">{fu.draft}</p>
                   <button 
                     onClick={() => handleWhatsApp(student.parentPhone, fu.draft || `Hi ${student.parentName}, checking in regarding: ${fu.objective}`)}
